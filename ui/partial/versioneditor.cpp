...
private:
	QVector<QPair<QString, Kdbx::XorredBuffer>> items;
public:
	...
	StringsModel(const std::map<std::string, Kdbx::XorredBuffer>& strings, QObject* parent)
		:QAbstractTableModel(parent)
	{
		setStrings(strings);
	}
	...
	void setStrings(const std::map<std::string, Kdbx::XorredBuffer>& strings){
		beginResetModel();
		items.clear();
		items.reserve(strings.size());

		for (const std::pair<std::string, Kdbx::XorredBuffer>& string: strings){
			...
			items.push_back(QPair<QString, Kdbx::XorredBuffer>(name, string.second));
		}
		endResetModel();
	}
	...
	std::map<std::string, Kdbx::XorredBuffer> strings(){
		std::map<std::string, Kdbx::XorredBuffer> result;
		for (QPair<QString, Kdbx::XorredBuffer>& string: items){
	...
	std::map<std::string, Kdbx::XorredBuffer> takeStrings(){
		std::map<std::string, Kdbx::XorredBuffer> result;
		beginResetModel();
		for (QPair<QString, Kdbx::XorredBuffer>& string: items){
			QByteArray name = string.first.toUtf8();
	...
	QModelIndex appendString(QString name = QString(), Kdbx::XorredBuffer value = Kdbx::XorredBuffer()) noexcept{
		if (!name.isEmpty() && hasString(name))
			return QModelIndex();
		size_t index = items.size();
		beginInsertRows(QModelIndex(), index, index);
		items.push_back(QPair<QString, Kdbx::XorredBuffer>(name, value));
		endInsertRows();
	...
	QVariant data(const QModelIndex &index, int role = Qt::DisplayRole) const override{
		...
		const QPair<QString, Kdbx::XorredBuffer>& entry = items.at(index.row());
		if (index.column() == 0){
			switch (role){
	...
	bool setData(const QModelIndex &index, const QVariant &value, int role = Qt::EditRole){
		...
		if (index.column() == 1){
			...
			items[index.row()].second = Kdbx::XorredBuffer(SafeVector<uint8_t>(data.begin(), data.end()));
			return true;
		}
...
void VersionEditor::fromVersion(QKdbxDatabase* dbx, const Kdbx::Uuid& uuid, const Kdbx::Database::Version* version){
	ui->db = dbx;
	...
	ui->iconButton->setDatabase(dbx);
	ui->iconButton->setCurrentIcon(version->icon);
...
//void VersionEditor::newVersion(QKdbxDatabase* db, const Kdbx::Uuid& uuid, Kdbx::Times times){
//	ui->db = db;
//	ui->times = times;
//	std::string suuid = std::string(uuid);
//	ui->uuidLabel->setText(QString::fromUtf8(suuid.c_str(), suuid.size()));
...
void VersionEditor::toVersion(Kdbx::Database::Version* version){
	...
	version->icon = ui->iconButton->currentIcon();
